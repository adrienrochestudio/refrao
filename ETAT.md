...
- **Flux connecté confirmé en prod le 2026-06-12** (gestionnaire démo + apprenant). Les anciens fichiers vanilla de la racine ont été **supprimés** (devenus morts). Le repo ne contient plus que l'app Astro (`app/`), l'outillage (`tools/`, `.github/`) et les docs/config. Rollback éventuel = `git revert` du commit de retrait + redéploiement (plus via la source Pages, la racine n'a plus de site vanilla).
- **Reste** : refonte CSS/UX (le `style.css` vit maintenant dans `app/public/`) ; nettoyage de la dette transitoire (handlers sur `window`) ; calibrer `leveltest` (stub) ; mettre à jour `README.md` (décrit encore le vanilla) ; plus tard RGPD + droits paroles, Stripe, domaine perso.
